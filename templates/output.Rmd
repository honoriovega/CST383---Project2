...
We see in this data set that PS2 has highest sum of sales of all platforms. While X360 and PS3 come in second and third respectively. Its suprising to see PS2 have the highest sum of sales since it was relased in March of 2000. 
```{r}
...
From our plot we see that games that achieved a critic score greater than 50 tended to have higher global sales. While majority of games that didn't achieve sales of 1 Million are distributed 20-100.
```{r}
...
### Cumlative Distribution of Global Sales(?)
```{r}
plot(ecdf(dat$global_sales), col="RED", main="Global Sales")
```
