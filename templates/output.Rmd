...
For each model we plot a histogram of its errors. This will allow us to get a visualization of the errors and compare our models to one another. We see with all 3 models the errors do not stray far from our predictions and that closely resemble one another. Regression trees are more prone to overfitting due it being a non-parametric model, but in this case our regression tree holds true since it similiar to our linear models.
```{r}
par(mfrow=c(1,3))
